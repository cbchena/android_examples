...
//        currentTime = (int) (60 / (2 * Math.PI) * currentRadian * 60);
        currentTime = (int) Math.round((60.0 / (2 * Math.PI) * currentRadian * 60));
        // Assist lines
//        canvas.drawColor(Color.RED);
    ...
    public void setTime(int second) {
        double angdeg = second / 10.0;
        currentRadian = (float) Math.toRadians(angdeg);
        invalidate();
    }
    ...
    public void stopTimer() {
        if (isStartTimer) {
            ...
            isStartTimer = false;
            if (this.circleTimerListener != null) {
                this.circleTimerListener.onTimerStop();
            }
    ...
    /**
     * 设置时间监听器 2015/6/9 14:34
     * @param circleTimerListener 监听器
     */
