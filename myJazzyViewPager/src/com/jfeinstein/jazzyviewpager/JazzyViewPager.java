...
import com.example.myJazzyViewPager.R;
...
//}
	...
	private void animateAccordion(View left, View right, float positionOffset) {
		if (mState != State.IDLE) {
			if (left != null) {
				manageLayer(left, true);
				ViewHelper.setPivotX(left, left.getMeasuredWidth()); // 设置中心为右上
				ViewHelper.setPivotY(left, 0);
				ViewHelper.setScaleX(left, 1-positionOffset);
			}
			if (right != null) {
				manageLayer(right, true);
				ViewHelper.setPivotX(right, 0); // 设置中心为左上
    ...
    /**
     * 自定义动画 2015/12/2 10:01
     * @param left 左视图
     * @param right 右视图
     * @param positionOffset 由右往左滑，值由0增加到1；反之，值取反；
     * @param positionOffsetPixels 移动的像素
     */
    private void animateCustom(View left, View right, float positionOffset, int positionOffsetPixels) {
        if (mState != State.IDLE) {
            if (left != null) {
                manageLayer(left, true);
                ViewHelper.setPivotX(left, left.getMeasuredWidth()*0.5f); // 设置中心原点 2015/12/2 9:51
                ViewHelper.setPivotY(left, left.getMeasuredHeight()*0.5f);
                if (positionOffset < 0.1 && positionOffset != 0) {
                    ViewHelper.setScaleX(left, 1 - positionOffset);
                    ViewHelper.setScaleY(left, 1 - positionOffset);
                } else if (positionOffset != 0) {
                    ViewHelper.setScaleX(left, 0.9f);
                    ViewHelper.setScaleY(left, 0.9f);
                } else {
                    ViewHelper.setScaleX(left, 1);
                    ViewHelper.setScaleY(left, 1);
                    ViewHelper.setTranslationX(left, 0);
                    ViewHelper.setTranslationY(left, 0);
                }

                // 将左视图置于顶部 2015/12/2 11:36
                left.bringToFront();
            }
            ...
            if (right != null) {
                ...
                ViewHelper.setPivotX(right, right.getMeasuredWidth()*0.5f);
                ViewHelper.setPivotY(right, right.getMeasuredHeight()*0.5f);
                if (positionOffset < 0.9 && positionOffset != 0) {
                    ViewHelper.setScaleX(right, 0.9f);
                    ViewHelper.setScaleY(right, 0.9f);
                } else if (positionOffset != 0) {
                    ViewHelper.setScaleX(right, positionOffset);
                    ViewHelper.setScaleY(right, positionOffset);
                } else {
                    ViewHelper.setScaleX(right, 0);
                    ViewHelper.setScaleY(right, 0);
                    ViewHelper.setTranslationX(right, 0);
                    ViewHelper.setTranslationY(right, 0);
                }

                // 维持在原位置不动 2015/12/2 11:22
...
//		mRight = getChildAt(position+1);
		...
		switch (mEffect) {
            ...
            case Accordion:
                ...
                break;
            case Custom:
                animateCustom(mLeft, mRight, effectOffset, positionOffsetPixels);
                break;
		}
